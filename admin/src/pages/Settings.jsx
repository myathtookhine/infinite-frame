...
const API_BASE_URL = 'http://localhost:5000/api/config';

const ConfigManager = ({ table, title, icon: Icon }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingItem) {
        // UPDATE (PUT)
        const response = await axios.put(`${API_BASE_URL}/${table}/${editingItem.id}`, { name: newItemName });
        if (response.data.success) {
          setItems(items.map(item => item.id === editingItem.id ? response.data.data : item));
        ...
        // CREATE (POST)
        const response = await axios.post(`${API_BASE_URL}/${table}`, { name: newItemName });
        if (response.data.success) {
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6 mb-8">
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0">
            ...
            <p className={`text-sm ${subtextColor} truncate`}>Manage gallery {title.toLowerCase()}</p>
          </div>
        </div>
        ...
        >
          ...
          <span className="whitespace-nowrap">Add {title}</span>
        </button>
      </div>
        ...
        <div className="w-full">
          ...
          <div className="space-y-3">
            {loading && items.length === 0 ? (
              ...
              <div className={`rounded-xl border-2 ${borderColor} ${cardBg} p-12 text-center`}>
                <p className={subtextColor}>No {title.toLowerCase()} found.</p>
              </div>
            ) : (
...
const Settings = () => {
  ...
  const tabs = [
    { id: 'categories', title: 'Categories', icon: ArchiveBoxIcon },
    { id: 'styles', title: 'Styles', icon: PaintBrushIcon },
    { id: 'themes', title: 'Themes', icon: SparklesIcon },
    { id: 'mediums', title: 'Mediums', icon: VariableIcon },
    { id: 'units', title: 'Units', icon: ArrowsPointingInIcon },
  ];
  ...
  return (
    <div className="max-w-7xl mx-auto">
      <div className="mb-10">
        <h1 className={`text-4xl font-sans font-black tracking-tight ${textColor} mb-2`}>Settings</h1>
        <p className={`${isDark ? 'text-gray-400' : 'text-gray-500'} font-sans`}>Configure your gallery's core taxonomies and settings.</p>
      </div>
